...
int ParsedRequest_unparse(struct ParsedRequest *pr, char *buf, 
			  size_t buflen, int relative)
{
     if (!pr || !pr->buf)
	  return -1;

     size_t tmp;
     if (ParsedRequest_printRequestLine(pr, buf, buflen, &tmp,relative) < 0)
	  return -1;
...
/* 
   ...
   Return values:
   -2: failure 501 Not Implemented
   -1: failure 400 Bad Request
   0: success
*/
int 
